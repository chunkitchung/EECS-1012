...
function createGameBoard(){
    ...
    for (var i = NUM_ATTEMPTS; i > 0; i--){
        ...
        for (var j = 1; j <= CODE_LENGTH; j++){
			...
			$(holeImg).attr("id", "attempt" + i +"_" +j);
			$(holeImg).attr("src", "./images/hole.png");
            $(newSpan).append(holeImg);
        }
		...
		$(resultSpan).attr("id", "attempt" + i +"result");
		$(newDiv).append(resultSpan);
    ...
    for (var i = 1; i <= NUM_BALLS; i++){
		...
		$(pegDiv).attr("id", "pegselection");
        var pegImg = document.createElement("img");
        $(pegImg).attr("id", "shadow" + i);
        $(pegImg).attr("src", "./images/shadow_ball_" + i +".png");
